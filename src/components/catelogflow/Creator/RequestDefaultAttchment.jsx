...
const RequestDefaultAttachment = ({
  selectedRecordId,
  catlogFlag,
  handleAttachFile
}) => {
  ...
  const uploadAttachmentsHandler = async (e, data) => {
    ...
    let allAttachment = [
      ...selectedFiles,
      { attachmentId: generateUId(), uploadResult }
    ];
    ...
    if (uploadResult) {
      handleAttachFile('attachment', allAttachment);
    }
  };
  ...
  //   deleteAttachment('system_catelogflow', 'attachment', attachment).then(
  //     (data) => {
  //       dispatch(
  //         setSnackBarAction({
  //           open: true,
  //           message: data.message || 'Attachment Download: Failed',
  ...
  const deleteSelectedFileHandler = (attachment) => {
    const updatedFiles = selectedFiles.filter(
      (o) => o.attachmentId !== attachment.attachmentId
    );
    setSelectedFiles(updatedFiles);

    handleAttachFile('attachment', updatedFiles);
  };
            ...
            <div className="flex flex-col items-center justify-start p-2">
              <div className="w-full">
                {selectedFiles.map((attachment, index) => (
                  <span key={index}>
                    {[...attachment.uploadResult].map((file, fileIndex) => (
                      <div
                      ...
                      >
                        <div
                          ...
                          style={{ width: '90%' }}
                        >
                          {file.fileContentType === 'text/html' ? (
                            <BsFiletypeHtml
                              style={{
                                fontSize: '30px',
                                marginRight: '7px',
                            ...
                            />
                          ) : file.fileContentType === 'application/json' ? (
                            <BsFiletypeJson
                              style={{
                                fontSize: '30px',
                                marginRight: '7px',
                            ...
                            file.fileContentType === 'image/jpeg' ||
                            file.fileContentType === 'image/x-icon' ? (
                            <LuImage
                              style={{
                                fontSize: '30px',
                                marginRight: '7px',
                            ...
                            />
                          ) : file.fileContentType === 'text/plain' ? (
...
export default RequestDefaultAttachment;
